	...
	var methods = {
		init: function(options) {
			return this.each(function() {
		        ...
		        // Added by Yash
				let sv_fields_error = [];
				$('fieldset.stepy-fieldset').each(function (key) {
					var note_errors = $('.note-error', this).length;
					var state_errors = $('.state-error', this).length;
					if (note_errors || state_errors) {
						sv_fields_error.push(key);
					}
				});
		        ...
		        $steps.each(function(index) {
		        	...
		        	title = $step.attr('stepy-title');
		        	title = (title != '') ? '<div>' + title + '</div>': '--';

					let originalFieldsetId = $step.data('id');

		        	$titlesWrapper.append('<li data-id="'+originalFieldsetId+'" id="' + id + '-title-' + index + '">' + title + description + '</li>');

		        	// Added by Yash
		        	if ($.inArray(index, sv_fields_error) != -1) {
		        		$('li#'+id + '-title-' + index).addClass('error');
		        		$('li#'+id + '-title-' + index+' div').append(' &nbsp;<span><i class="fa fa-2x fa-times-circle"></i></span>');
		        	}
				...
				let hashToStart = window.location.hash.substring(1)
				if (hashToStart.length) {
					let startingIndex = $titlesWrapper.find("[data-id='fieldset_" + hashToStart + "']").index()

					let $relatedFieldset = $this.children('fieldset').eq(startingIndex);
					let fieldsetHasContent = $relatedFieldset.find('div.sv-row').length
					if (startingIndex >= 0 && fieldsetHasContent) {
						methods.step.call($this, startingIndex + 1);
					}
				}
				methods.disableTitlesRelatedToEmptyFieldsets.call($this)
        	...
        	// avoid stepping if content is empty
			let fieldsetHasContent = this.children('fieldset').eq(index).find('div.sv-row').length
	        ...
	        return this;
		}, disableTitlesRelatedToEmptyFieldsets: function() {
			let $steps = this.children('fieldset');
			let $titles	= $('#' + this.attr('id') + '-titles').children();

			$steps.each(function(subindex) {
				let fieldsetHasContent = $(this).find('div.sv-row').length
				if (!fieldsetHasContent) {
					$titles.eq(subindex).addClass('content-less-fieldset')
						.attr({'data-toggle': 'tooltip', 'data-original-title': 'Questo ruolo é privo di campi propri'});
				}
			});
